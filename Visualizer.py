...
if __name__ == '__main__':
    ...
    for model_info in model_infos:
        if model_info['name'] == 'ResNet' and model_info['num_classes'] == 3 and model_info['augmentation'] == 'norm' and model_info['transfer'] == 'retrain':
            epochs = np.arange(1, len(model_info['val_losses']) + 1)
            chart_val_losses = model_info['val_losses']
            chart_val_accs = model_info['val_accs']
            chart_test_losses = [model_info['test_loss'] for _ in range(len(model_info['val_losses']))]
            chart_test_accs = [model_info['test_acc'] for _ in range(len(model_info['val_accs']))]
            break

    fig = plt.figure()
    ax = plt.subplot(2, 1, 1)
    plt.plot(epochs, np.transpose([chart_val_losses, chart_test_losses]))
    plt.ylabel('Validation loss during training')
    ax = plt.subplot(2, 1, 2)
    plt.plot(epochs, np.transpose([chart_val_accs, chart_test_accs]))
    plt.ylabel('Validation accuracy during training')
    plt.xlabel('Epochs')
    ...
    chart_test_losses = []
    for model_info in model_infos:
        if model_info['medical_mode'] == 'SA' and model_info['name'] == 'ResNet' and model_info['num_classes'] == 3 and model_info['augmentation'] == 'norm' and model_info['transfer'] == 'retrain':
